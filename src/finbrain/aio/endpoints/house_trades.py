...
class AsyncHouseTradesAPI:
    ...
    async def ticker(
        ...
        *,
        ...
        ``"SP"`` (spouse), ``"DC"`` (dependent child), ``"JT"`` (joint), or
        an account code; House filings that leave the owner column blank
        report as ``"SELF"``. ``amountRaw`` preserves the originally filed
        amount string on rows whose ``amount`` was normalized to a STOCK Act
